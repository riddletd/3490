...
/* Start and end state generators. These allow us to get these states
   into variables and compare them, examine them, etc. */
initial([e,e,e,e]).
final([w,w,w,w]).
...
/* Program entry point */
fwgc :-
      initial(Start),                /* generate our starting state */
      length(Solution, L), L < 10,   /* let prolog make a "guess" */
      verify([Start|Solution]),      /* verify the guess ensuring we start
                                        where we are supposed to! */
      prettyprint([Start|Solution]). /* only get here if guess verified! */


/* Solutions are being generated so verify a solution.
 * recursive case: get a list of states, make sure can move from first 
 *                 to second state, then recurse on path without first state.
 */
...
verify([State1, State2 | Tail]) :-
 ...
 /* base case: list of one state that is the final state. */
